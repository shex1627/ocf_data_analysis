...
3. Exclude sessions from host "blizzard" and "eruption" because they are desktops exclusive to front desk staff members and volunteer staff memebers. 
...
```{r corrupted_session_plot, eval=F}
...
ggplot(corrupted_sessions) +
  ...
  ylim(0, 1.05) +
  theme(legend.position = "none") +
  labs(title="Percent of Inaccurate Sessions Over Time ", y ="percent") +
...
median_plot = ggplot(temp_df %>% head(5)) +
  ...
  geom_text(aes(x=t, y=median_wait, label=round(median_wait,1)), 
            check_overlap = TRUE,
            vjust = -1) + 
  labs(title="Median Remaining Session Duration vs. Current Session Duration", x="current Session time", y="duration in minutes") +
